...
// 而标准库的 free 和 realloc 函数却不给出内存的大小。 所以大多数内存管理模块在实现时， 在内存块的前面加了一个 cookie ，把内存块尺寸存放在里面。
// lua 在实现时，刻意提供了这个信息。 这样我 们不必额外储存内存块尺寸，这对大量使用小内存块的环境可以节约不少内存
/**
 * realloc(ptr, nsize)
 ...
 */
// 内存管理函数  第1,3个参数没用到
static void *l_alloc (void *ud, void *ptr, size_t osize, size_t nsize) {  // 管理内存的时候,给出内存的大小;
  (void)ud; (void)osize;  /* not used */
  if (nsize == 0) {  // 在 nsize 为 0 时，提供释放内存的功能。 ?? 在释放的时候也没有给出内存大小呀
    free(ptr);
    return NULL;
  }
  else
...
LUALIB_API lua_State *luaL_newstate (void) {  // 创建虚拟机
  lua_State *L = lua_newstate(l_alloc, NULL);  // 传入内存管理函数
